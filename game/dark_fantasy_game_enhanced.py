...
# Import the enhanced game state
try:
    from dark_fantasy_game.src.game_state_enhanced import GameState
except ImportError as e:
    print(f"Error importing game state: {e}")
    print("Falling back to basic game state...")
    try:
        from dark_fantasy_game.src.game_state import GameState
    except ImportError:
        print("Could not import any game state. Please check your installation.")
        sys.exit(1)
...
class Game:
    def __init__(self):
        # Khởi tạo màn hình với chế độ có thể thay đổi kích thước
        try:
            self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.RESIZABLE)
            pygame.display.set_caption("Dark Fantasy Stickman - Enhanced Edition")
    ...
    def auto_adjust_screen_size(self):
        ...
        try:
            ...
            # Đảm bảo tỷ lệ khung hình 4:3 hoặc 16:9
            if optimal_width / optimal_height > 1.78:  # Nếu quá rộng (> 16:9)
                optimal_width = int(optimal_height * 1.78)
            elif optimal_width / optimal_height < 1.33:  # Nếu quá cao (< 4:3)
                optimal_height = int(optimal_width / 1.33)
                
            # Cập nhật kích thước màn hình
            self.resize(optimal_width, optimal_height)
            self.last_windowed_size = (optimal_width, optimal_height)
        except Exception as e:
            print(f"Error adjusting screen size: {e}")
        ...
        try:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # Xử lý khi người dùng thay đổi kích thước cửa sổ (chỉ khi không ở chế độ toàn màn hình)
                    self.resize(event.w, event.h)
                    self.last_windowed_size = (event.w, event.h)
                        
                # Chuyển sự kiện đến game state với tỷ lệ tương ứng
                if event.type == pygame.MOUSEBUTTONDOWN:
                    # Không điều chỉnh vị trí chuột, sử dụng vị trí thực tế
    ...
    def resize(self, width, height):
        try:
            ...
            print(f"Resized to: {width}x{height}, Scale factors: {self.scale_factor_x:.2f}x, {self.scale_factor_y:.2f}y")
        except Exception as e:
    ...
    def draw(self):
        try:
            ...
            # Hiển thị thông tin debug về kích thước màn hình (có thể xóa trong phiên bản cuối cùng)
            debug_font = pygame.font.SysFont(None, 20)
            debug_text = debug_font.render(
                f"Screen: {self.screen.get_width()}x{self.screen.get_height()} | Scale: {self.scale_factor_x:.2f}x, {self.scale_factor_y:.2f}y", 
                True, (100, 100, 100))
            self.screen.blit(debug_text, (5, 5))
